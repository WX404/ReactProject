    ...
    componentDidMount() {
        ...
        $.ajax({
            ...
            success(res) {
                ...
                that.setState({
                    imgData: res[0],
                    commentList: res.slice(1, endIndex)
                })
                console.log('imgData', that.state.imgData)
                console.log('commentList', that.state.commentList)
            }
        })
    }
    toHome() {
        ...
        if (username) {
            if (comment) {
                $.ajax({
                    ...
                    success(res) {
                        let comments = that.state.commentList;
                        comments.push({ nickname: nickname, comment: comment, time: commentTime })
                        that.setState({
            ...
            $(".comment-login-tip").css("display", "block")
            setTimeout(() => {
                $(".comment-login-tip").css("display", "none")
            }, 1000)
        }
    }
    commentList() {
        ...
        for (let i = 0; i < commentList.length; i++) {
            listItems.push(<div className="commentlist-item" key={i}><p>{sessionStorage.getItem("nickname")}</p>
        ...
        return (
            <div className="singleimg">
                ...
                <div className="singleimg-comment">
                    <p>评论</p>
                    <TextArea className="comment-textarea" onChange={(event) => { this.Comment(event) }} />
                    <div className="comment-release" onClick={this.commentRelease.bind(this)}>发布</div>
                    <div className="comment-tip">内容不能为空</div>
                    <div className="comment-login-tip">评论请先登录哦</div>
                </div>
